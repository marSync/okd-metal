...
curl -L https://github.com/okd-project/okd/releases/download/$OKD_VERSION/openshift-client-linux-$POST_ARCH-$OKD_VERSION.tar.gz -o $OC_TAR_NAME
tar zxf $OC_TAR_NAME
chmod +x oc

# Download the OKD installer and make it available for use by entering the following commands
curl -L https://github.com/okd-project/okd/releases/download/$OKD_VERSION/openshift-install-linux-$POST_ARCH-$OKD_VERSION.tar.gz -o $INSTALLER_TAR_NAME
